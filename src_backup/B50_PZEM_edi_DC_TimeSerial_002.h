...
#define B50_PZEM_ID		   		42  					// this is a unique PZEM ID for reference, it is NOT slave MODBUS address, just some random id
						   								// (we all know why '42' is THE number, right? :) )

// first, we need a UartQ object to handle RX/TX message queues
UartQ *g_B50_UartQueue;
...
void B50_PZEM_Init() {
	...
	g_B50_UartQueue = new UartQ(PZEM_UART_PORT, RX_PIN, TX_PIN);  // or use custom pins
	...
	//g_B50_PZ004	= new PZ004(B50_PZEM_ID);
	...
	// g_B50_PZ003->attach_rx_callback([ref](uint8_t p_PZEM_id, const RX_msg* p_PZEM_RX_msg) {
	// 	B50_PZEM_rx_callback(p_PZEM_id, p_PZEM_RX_msg);
	// });

	g_B50_PZ003->attach_rx_callback([ref](uint8_t pzid, const RX_msg *m) {
		auto *data = g_B50_PZ003->getMetricsPZ003();			 		// obtain a pointer to objects metrics
		ref->push(*data, esp_timer_get_time() >> 20);	 				// push data to TS container and mark it with "seconds' timer 데이터를 TS 컨테이너에 푸시하고 '초' 타이머로 표시합니다.
	});

	// g_B50_PZ004->attach_rx_callback([ref](uint8_t pzid, const RX_msg *m) {
