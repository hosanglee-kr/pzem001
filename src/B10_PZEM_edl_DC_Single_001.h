...
void B10_PZEM_rx_callback(uint8_t p_PZEM_id, const RX_msg* p_PZEM_RX_msg);
...
#define B10_PZEM_UART_PORT UART_NUM_1  // port attached to pzem (UART_NUM_1 is 2-nd uart port on ESP32, 1-st one is usually busy with USB-to-serial chip)

#define RX_PIN			   22  // custom RX pin number
#define TX_PIN			   19  // custom TX pin number

#define B10_PZEM_ID		   42  // this is a unique PZEM ID for reference, it is NOT slave MODBUS address, just some random id (we all know why '42' is THE number, right? :) )

UartQ* g_B10_UartQueue;	 // first, we need a UartQ object to handle RX/TX message queues

PZ003* g_B10_PZ003;	 // Also we need a placeholder for PZEM object


// g_B10_PZ003->resetEnergyCounter() 
...
void   B10_Init() {
	...
	g_B10_UartQueue = new UartQ(B10_PZEM_UART_PORT, v_uart_cfg, RX_PIN, TX_PIN);	// or use custom pins
	...
	g_B10_PZ003	  = new PZ003(B10_PZEM_ID, ADDR_ANY,"PZEM003-01"); //char[9]);   // i.e. PZEM-123

	// and link our port with PZEM object
	g_B10_PZ003->attachMsgQ(g_B10_UartQueue);

	// one last step - we must start PortQ tasks to handle messages
	g_B10_UartQueue->startQueues();

	// set shunt type
	g_B10_PZ003->setShunt(shunt_t::type_50A);

	// now it is all ready to exchange data with PZEM
	// let's update it's metrics
	g_B10_PZ003->updateMetrics();

	// and try to check the voltage value
	auto* m = g_B10_PZ003->getMetricsPZ003();	 // obtain a pointer to objects metrics
	...
	if (g_B10_PZ003->autopoll(true)) {
		Serial.println("Autopolling enabled");
	} else {
		Serial.println("Sorry, can't autopoll somehow :(");
	}

	//g_B10_PZ003->getPollrate();
	//g_B10_PZ003->setPollrate(1000);	// 1000ms
...
void B10_PZEM_rx_callback(uint8_t p_PZEM_id, const RX_msg* p_PZEM_RX_msg) {
	...
	//	Serial.printf("PZEM current as float: %.3f (Amps)\n", g_B10_PZ003->getMetricsPZ003()->asFloat(pzmbus::meter_t::cur));
