...
uint8_t						g_B51_TsC_ID_sec1;
uint8_t						g_B51_TsC_ID_sec30;
uint8_t						g_B51_TsC_ID_sec300;
...
void B51_setup_timeseries(struct timeval* t) {
	...
	g_B51_TsC_ID_sec1 = g_B51_TsC_pz004.addTS(
									  300						//
									, time(nullptr) 			// current timestamp
									, 1 						// second interval
									, "TimeSeries 1 Second" 	// Mnemonic descr
								);
	...
	g_B51_TsC_ID_sec30 = g_B51_TsC_pz004.addTS(
									  240
									, time(nullptr) 			// current timestamp
									, 10 						// second interval
	...
	g_B51_TsC_ID_sec300 = g_B51_TsC_pz004.addTS(
									  288
									, time(nullptr) 			// current timestamp
									, 300 						// second interval
	...
	auto v_TsC_pz004_ref = &g_B51_TsC_pz004;  // a ref of our Container to feed it to lambda function
...
void B51_print_timeseries() {
	...
	auto   v_ts	= g_B51_TsC_pz004.getTS(g_B51_TsC_ID_sec1);
	...
	for (v_iter; v_iter != v_ts->cend(); ++v_iter) {
		...
		Serial.print(v_timestamp_lastupdate);
		Serial.print("\t");
	...
	v_ts	 = g_B51_TsC_pz004.getTS(g_B51_TsC_ID_sec30);
	v_iter 	 = v_ts->cbegin();
	...
	for (v_iter; v_iter != v_ts->cend(); ++v_iter) {
		...
		char*		v_dtime 				= std::ctime(&v_timestamp_lastupdate);

		Serial.println("Date/time\tdV\tmA\tW\tWh\tdHz\tpf");
		Serial.print(v_dtime);
		Serial.print("\t");
		Serial.print(v_iter->voltage);
		Serial.print("\t");
		Serial.print(v_iter->current);
