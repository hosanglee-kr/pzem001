...
void B70_PZEM_CLI_menu();
void get_addr_bcast();
void B70_UartQue_rx_handler(RX_msg* m);
void set_mbus_addr();
void get_metrics();
void reset_nrg();
void get_alrm_thr();
...
void   B70_PZEM_CLI_init() {
	...
	qport->attach_RX_hndlr([](RX_msg* msg) { 
			B70_UartQue_rx_handler(msg); 
			delete msg; 
	});	// attach call-back function to process pzem replies

	// now we are ready to exchange messages
}

void B70_PZEM_CLI_run() {
	B70_PZEM_CLI_menu();	 // print menu in a loop
}
